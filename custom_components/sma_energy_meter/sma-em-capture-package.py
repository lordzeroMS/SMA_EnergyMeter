...
sensors = []
for k in emparts.keys():
    sensor = {}
    if k.endswith('unit'):
        continue
    unit = emparts.get(k + 'unit', None)
    if unit is None or unit in ('°'):
        sensor['_attr_device_class'] = None
    elif unit in ('V'):
        sensor['_attr_device_class'] = SensorDeviceClass.VOLTAGE
    elif unit in ('W', 'A'):
        sensor['_attr_device_class'] = SensorDeviceClass.POWER
    elif unit in ('VA', 'VAr'):
        sensor['_attr_device_class'] = SensorDeviceClass.APPARENT_POWER
    elif unit in ('Hz'):
        sensor['_attr_device_class'] = SensorDeviceClass.FREQUENCY
    ...
    sensor['_sensor_type'] = k
    sensor['_value'] = emparts[k]
    sensor['human_readable_value'] = f"{sensor['_value']} {sensor['_attr_unit_of_measurement'] if sensor['_attr_unit_of_measurement'] else ''}".strip()
    sensors.append(sensor)

print ('----all-found-sensors---')
for s in sensors:
    print (s)


print ('----all-found-values---')
for k,v in emparts.items():
